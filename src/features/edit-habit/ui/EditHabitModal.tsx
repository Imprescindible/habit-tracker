...
const COLORS  = ['#E94560','#7C3AED','#06BCC1','#F59E0B','#10B981','#3B82F6']
const DAYS_RU = ['Пн','Вт','Ср','Чт','Пт','Сб','Вс']

interface Props { habit: Habit; onClose: () => void }
...
export function EditHabitModal({ habit, onClose }: Props) {
  ...
  const [scheduleInterval, setScheduleInterval] = useState(String(habit.scheduleInterval ?? 2))
  const [color,            setColor]            = useState(habit.color)
  const [confirmDelete,    setConfirmDelete]    = useState(false)
  const [loading,          setLoading]          = useState(false)

  const toggleDay = (day: number) =>
    setScheduleDays((p) => p.includes(day) ? p.filter((d) => d !== day) : [...p, day])
  ...
  const handleSave = async () => {
    ...
    try {
      const updated = await habitApi.update({
        ...
        scheduleInterval: scheduleType === 'INTERVAL' ? Number(scheduleInterval) : null,
        color,
      })
      updateHabit(updated)
      onClose()
  ...
  return (
    <Modal isOpen title="Редактировать привычку" onClose={onClose}>
      <div className={styles.form}>
        ...
        <Textarea label="Описание" rows={2} value={description} onChange={(e) => setDescription(e.target.value)} />

        <div className={styles.field}>
          <span className={styles.label}>Тип</span>
          <div className={styles.toggleGroup}>
            {(['BINARY','QUANTITATIVE'] as HabitType[]).map((t) => (
              <button key={t} className={`${styles.toggleBtn} ${type === t ? styles.active : ''}`}
                onClick={() => setType(t)}>
        ...
        {type === 'QUANTITATIVE' && (
          <div className={styles.row}>
            <Input label="Цель" type="number" value={goalValue} onChange={(e) => setGoalValue(e.target.value)} />
            <Input label="Единица" value={goalUnit} onChange={(e) => setGoalUnit(e.target.value)} />
          </div>
        )}

        <Select label="Время дня" value={timeSlot} onChange={(e) => setTimeSlot(e.target.value as TimeSlot)}>
        ...
        <Select label="Расписание" value={scheduleType} onChange={(e) => setScheduleType(e.target.value as ScheduleType)}>
        ...
        {scheduleType === 'SPECIFIC_DAYS' && (
          <div className={styles.field}>
            ...
            <div className={styles.daysGrid}>
              {DAYS_RU.map((day, i) => (
                <button key={i} className={`${styles.dayBtn} ${scheduleDays.includes(i+1) ? styles.dayActive : ''}`}
                  onClick={() => toggleDay(i+1)}>{day}</button>
              ))}
            </div>
          </div>
        )}
        ...
        {scheduleType === 'INTERVAL' && (
          <Input label="Раз в N дней" type="number" min={1} value={scheduleInterval}
        ...
        <div className={styles.actions}>
          <div className={styles.danger}>
            <Button variant="ghost" size="sm" onClick={handleArchive} disabled={loading}>Архивировать</Button>
